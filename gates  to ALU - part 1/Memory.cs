...
namespace Components
{
    ...
    class Memory : SequentialGate
    {
        ...
        public Memory(int iAddressSize, int iWordSize)
        {
            ...
            //your code here
            MultiWayDemux demux = new MultiWayDemux(iAddressSize);
            demux.ConnectControlers(Address);
            demux.ConnectInput(Load);
            BitwiseMultiwayMux mux = new BitwiseMultiwayMux(iWordSize, iAddressSize);
            mux.ConnectControl(Address);
            MultiBitRegister[] arrReg = new MultiBitRegister[(int)Math.Pow(2, iAddressSize)];
            for(int i=0; i< arrReg.Length; i++)
            {
                MultiBitRegister reg = new MultiBitRegister(iWordSize);
                reg.Load.ConnectInput(demux.Output[i]);
                reg.ConnectInput(Input);
                mux.ConnectInput(i,reg.Output);
            }
